        ...
        for fila in lector:
            if fila["espacio_ve"].strip().upper() == nombre_parque.strip().upper():
                ...
                arbol_info = {
                    ...
                    if clave not in ("coord_x", "coord_y")
                }  # ignoro ultimas dos columnas
...
def n_mas_altos(nombre_parque, n):
    ...
    arboles_ordenados = sorted(
        arboles.items(), key=lambda item: int(item[1]["altura_tot"]), reverse=True
    )
...
def parques_mas_arboles(nombre_archivo):
    ...
    parques_maximos = [
        parque for parque, cantidad in contador.items() if cantidad == max_cantidad
    ]
...
def parques_mas_altos_promedio(nombre_archivo):
    ...
    parques_maximos = [
        parque for parque, promedio in promedios.items() if promedio == max_promedio
    ]
...
def parques_mas_variedad(nombre_archivo):
    ...
    parques_maximos = [
        parque
        for parque, cantidad in cantidad_por_parque.items()
